...
import pandas as pd
from src.utils.config import config, DRIVERS_FILE
...
def manage_drivers():
    ...
    # Load driver data
    driver_data = config.config.load_json_data(DRIVERS_FILE)
    if not driver_data:
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                if st.button("Import and Update Drivers"):
                    ...
                    if updates_made > 0:
                        if config.config.save_json_data(driver_data, DRIVERS_FILE):
