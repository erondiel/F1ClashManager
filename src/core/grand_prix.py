...
from src.utils.config import config, LOADOUTS_FILE, DRIVERS_FILE, COMPONENT_FILE
from src.utils.utils import format_number, safe_float, safe_int, get_current_timestamp
...
def load_gp_loadouts():
    ...
    gp_loadouts_data = config.config.load_json_data(GP_LOADOUTS_FILE)
    if not gp_loadouts_data:
        # Create default structure if file doesn't exist
        gp_loadouts_data = {"grand_prix_loadouts": []}
        config.config.save_json_data(gp_loadouts_data, GP_LOADOUTS_FILE)
...
def get_component_series(component_name):
    ...
    for component_type, file_path in COMPONENT_FILES.items():
        components_data = config.config.load_json_data(file_path)
        if components_data:
...
def get_driver_series(driver_name):
    ...
    drivers_data = config.config.load_json_data(DRIVERS_FILE)
    if drivers_data:
...
def display_category_loadouts(category, gp_loadouts):
    ...
    if selected_gp_loadout:
        ...
        with col2:
            st.write("#### Components")
            component_types = {"brakes": "Brakes", "gearbox": "Gearbox", "rear_wing": "Rear Wing", 
                             "front_wing": "Front Wing", "suspension": "Suspension", "engine": "Engine"}
            
            for comp_type, comp in selected_gp_loadout["components"].items():
                if comp["name"]:
                    display_name = comp["name"]
                    display_type = component_types.get(comp_type, comp_type.title())
                    st.write(f"{display_type}: **{display_name}** (Level {comp['level']}, Series {comp.get('series', '?')})")
