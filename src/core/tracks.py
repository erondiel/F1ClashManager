...
import pandas as pd
from src.utils.config import config, TRACK_BOOSTS_FILE, BOOSTS_FILE, SERIES_SETUPS_FILE
from src.utils.utils import safe_float, safe_int
...
def manage_tracks():
    ...
    tracks_data = config.config.load_json_data(TRACK_BOOSTS_FILE)
    boosts_data = config.config.load_json_data(BOOSTS_FILE)
...
def manage_series_setups():
    ...
    data = config.config.load_json_data(SERIES_SETUPS_FILE)
